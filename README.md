...
27-cell neighborhood sum: Each cell's next state depends on the sum of itself plus its 26 Moore neighbors (all 27 cells in a 3×3×3 cube)
8-bit rule space: Only neighbor sums 0-7 are addressable; sums ≥8 always produce dead cells
State-independent update: Next state depends only on the neighbor sum, not whether the cell is currently alive
...
Wolfram's canonical 3D totalistic rules (Demonstrations Project) use:
...
pythonfrom src.core import analyze_rule_systematic
...
bashpython examples/run_all_256.py
```
...
Result: Rule 54 produces living cells for sums of 1, 2, 4, or 5. All cells with sum ≥8 die.
Update Algorithm
python# 1. Create 27-cell neighborhood kernel (3×3×3 cube)
...
Canonical 3D totalistic: Implement true 26-neighbor (self-excluded) rules with 27-bit masks
Outer-totalistic (Life-like): Add birth/survival distinction based on current state
Extended count ranges: Map counts 0-26 to 8 bins via modulo or binning functions
Sparse rule sampling: Heuristically explore the 2^27 canonical space
3D Life variants: Implement known 3D Life rules for comparison